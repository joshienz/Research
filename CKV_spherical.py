...
def main():
    ...
    big_T = np.zeros((int(mat_sz),int(mat_sz)),dtype=np.complex128)
    M00 = np.zeros((int(mat_sz),int(mat_sz)),dtype=np.complex128)
    ...
    for l in range(lmin,lmax+1,step):
        ...
        for col in range ((mat_sz),nbasis+(mat_sz)):
            ...
            #print(l,sarg_vec[col-(mat_sz)],res0_MQ0_i[0])
    ...
    #M_inv = np.linalg.solve(M_mat,np.eye(26))
   
    #debugging
    if debug:
        print("M00 ",M00)
        print("MQ0 matrix ",S_mat)
        print("MQ0 matrix T",S_T)
        print("MQQ ",M_mat)
        #print("M_inv ",M_inv)
        #print("Seudo M_inv",SM_inv)
        #print("I",M_inv@M_mat)
        #print("Det",np.linalg.det(M_mat))
        
    M_inv= np.linalg.inv(M_mat)
    SM_inv = np.linalg.pinv(M_mat)
    ...
    print(lmbda_ah)
    print("\t{}".format(math.atan(lmbda_est)))
    '''
    file.write("{}".format(k))
