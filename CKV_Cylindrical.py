...
alpha_standards = [ [5,2.5,1.25,0.625,0.3125,0.15625,0.078125],#,0.0390625,0.1953125,0.009765625,0.0048828125,0.0024414063,0.001220703,0.0006103516],
                   [10,3.3333333,1.111111111,0.37037037,0.12345679,0.0411522634],#,0.0137174211,0.00457247],
                   [6,2,0.6666667,0.22222222222,0.07407407,0.024691358,0.0082304527],
                   [4,2,1,0.5],#0.3125,0.15625,0.078125],#,0.1666666666],#,0.055555556]
                   [3,1.5]
                   ]
...
def legendre_poly(l,m,z,r,rho):
    ...
    l : Quantum number l
    m : Quantum number m
    z, r, rho : Since the legendre polynomial uses cos(theta) and our code 
    does not have a theta we use these three parameters to calculate cos and
    sin(theta)
...
def M00_integrand(params):
    ...
    rho : Argument integrating over from 0 to inf
    z : second argument integrating over from 0 to inf
    k : Momentum constant
    ol : L for the right bessel fnct
    ol_prime : L for the left bessel fnct
    om : m for Bessel fncts
    ...
    """
    #unpack params
    z,rho,k,ol,ol_prime,om,z_atom1,z_atom2=params
    ...
    Legendre = legendre_poly(l,m,z,r,rho )
    Legendre_prime = legendre_poly(l_prime,m,z,r,rho )
    #calculate the integrand
    integ = 4*PI*rho*J_prime*V*J*Legendre*Legendre_prime/k
    return integ
#M00_integrand
...
def MQ0_0_integrand(params):
    ...
    ol : l for the bessel fnct
    ol_prime : L for the hankle fnct
    om : m for the psherical fncts
    z_atom1, z_atom2 : z-coords of the two atoms
    ...
    """
    #unpack params
    z,rho,b,k, c,ol,ol_prime,om,z_atom1,z_atom2 = params
    #numba causes all params to be floats so convert to ints
    l = int(ol+int_c)
    m = int(om+int_c)
    l_prime=int(ol_prime+int_c)
    ...
    integ = 4*PI*G*H*V*J*Legendre*rho*Legendre_prime/k
...
def MQ0_i_integrand(args):
    ...
    """
    #unpack params
    z,rho,ol,om,b,k,alpha,c,sym,l1,m1,z_atom1,z_atom2 = args
    #numba causes all params to be floats so convert to ints
    l= int(ol + int_c)
    m = int(om + int_c)
    l1 = int(l1+int_c)
    ...
    for i in range (0, numberOfTerms[l1][m1+l1]):
        ...
        Rho = rho**(x_pow+y_pow)
        Q += (coeff_b1*Rho*(phi_re+phi_im*1j)*
    ...
    integ= 2*Q*V*J*rho*Legendre/np.sqrt(k)
...
def MQQ_00_integrand(params):
    ...
    k : Momentum constant
    c : 0 or 1 used to control the return of real or imaginary part
    ol : L for the right hankle fnct
    ol_prime : L for the left hankle fnct
    om : m for the hankle functions
    z_atom1, z_atom2 : z-coords of the two atoms
    ...
    integ = 4*PI*Legendre_prime*(G_prime*H_prime)*((-1/2)*G_dub_dot*H-G_dot*(k*H_dot+H/r)+V*H*G)*Legendre*rho/k
...
def MQQ_i0_integrand(args):
    ...
    for i in range (0, numberOfTerms[l1][m1+l1]):
        ...
        (np.exp(-alpha*rasq)*za_diff**z_pow+
         combo_sign1*np.exp(-alpha*rbsq)*zb_diff**z_pow))
    #integrand contains potentail and terms from second deriv of G*H after cancellation
    integ = 2*(Q/np.sqrt(k))*((-1/2)*G_dub_dot*H-G_dot*(k*H_dot+H/r)+V*H*G)*rho*Legendre
...
def main():
    ...
    k=.447#momentum constsant
    lmax=7#maximum L to loop over
    odd = 1 #make 1 if lmax is odd
    m=0     #Quantum number m based on symmetry
    ...
    debug = True #print out matricies 
    ...
    M_mat =np.zeros((nbasis+(1+mat_sz),nbasis+(1+mat_sz)),dtype=np.complex128) 
    S_mat =np.zeros((mat_sz+1+nbasis,(1+mat_sz)),dtype=np.complex128)
    ...
    for row in range (0, nbasis):
        for col in range (0, row+1):
           ...
           M_mat[row+(1+mat_sz),col+(1+mat_sz)]= (T[row,col]+N[row]*N[col]*(res0_MQQ_ij[0]-(k**2/2)*S[row,col]))+0j
           M_mat[col+(1+mat_sz),row+(1+mat_sz)]= M_mat[row+(1+mat_sz),col+(1+mat_sz)]
           counter= counter+1
    
 
    #create the T matrix and M00
    big_T = np.zeros((int(lmax/step+1),int(lmax/step+1)),dtype=np.complex128)
    M00 = np.zeros((int(lmax/step+1),int(lmax/step+1)),dtype=np.complex128)
    
    #Big loop which calls everything and finds all the different pieces 
        #of the matricies to get a phase shift
    for l in range(odd,lmax+1,step):
        ...
        for basisfunction1 in range(0,nbasis):
                ...
                sarg_vec.append(list(arguments))
                
        for l_prime in range(odd,l+1,step):
            arg_vec = []
            arg_vec_im = []
            arg_vec_prime = []
            arg_vec_im_prime = []
            #this is the argument list created for MQQ_i0
            for basisfunction1 in range(0,nbasis):
                # Each basisfunction is given by a row of [l, m, alpha] in SymSet.
                lma1 = SymSet[basisfunction1] 
                l1 = lma1[0]
                ...
                alpha1 = lma1[2]
                # sarg_vec is a list of lists. Each list contains:
                    # an lor l_prime and m for symmetry type
                    # a B for the cutoff fnct
                    # a k constant
                ...
                arguments=[l_prime,m,B,k,alpha1,0,sym,l1,m1,z_atom1,z_atom2]
                arg_vec_prime.append(list(arguments))
                arguments=[l,m,B,k,alpha1,1,sym,l1,m1,z_atom1,z_atom2]
                arg_vec_im.append(list(arguments))
                arguments=[l_prime,m,B,k,alpha1,1,sym,l1,m1,z_atom1,z_atom2]
                arg_vec_im_prime.append(list(arguments))
            
            # MQQ at i=j=0 elem real and imag parts
            args_MQQ_00=(B,k,0,l,l_prime,m,z_atom1,z_atom2)    
            res0_MQQ_00_real = nquad(MQQ_00_integrand, [(0.0, np.inf),(0.0,np.inf)], args_MQQ_00) 
            args_MQQ_00=(B,k,1,l,l_prime,m,z_atom1,z_atom2)    
            res0_MQQ_00_imag = nquad(MQQ_00_integrand, [(0.0, np.inf),(0.0,np.inf)], args_MQQ_00) 
            M_mat[int(l/step),int(l_prime/step)] =res0_MQQ_00_real[0] + 1j*res0_MQQ_00_imag[0]
            args_MQQ_00=(B,k,0,l_prime,l,m,z_atom1,z_atom2)    
            res0_MQQ_00_real = nquad(MQQ_00_integrand, [(0.0, np.inf),(0.0,np.inf)], args_MQQ_00) 
            args_MQQ_00=(B,k,1,l_prime,l,m,z_atom1,z_atom2)    
            res0_MQQ_00_imag = nquad(MQQ_00_integrand, [(0.0, np.inf),(0.0,np.inf)], args_MQQ_00) 
            M_mat[int(l_prime/step),int(l/step)] =res0_MQQ_00_real[0] + 1j*res0_MQQ_00_imag[0]
             

        #     
        # =============================================================================
            #diagonally symeytric so only need one half 
            for col in range ((1+mat_sz),nbasis+(1+mat_sz)):
                #inserting Edge elements to MQQ matrix
                res0_MQQ_i0_real =  nquad(MQQ_i0_integrand, [(0.0, np.inf),(0.0,np.inf)], arg_vec[col-(1+mat_sz)]) 
                res0_MQQ_i0_imag =  nquad(MQQ_i0_integrand, [(0.0, np.inf),(0.0,np.inf)], arg_vec_im[col-(1+mat_sz)])
                #print("i0",res0_MQQ_i0_real[0] + 1j*res0_MQQ_i0_imag[0])
                M_mat[int(l/step),col]=(res0_MQQ_i0_real[0] + 1j*res0_MQQ_i0_imag[0])*N[col-(1+mat_sz)]
                M_mat[col,int(l/step)]=M_mat[int(l/step),col]
   
            #build M00 matrix 
            args_M00 =(k,l,l_prime,m,z_atom1,z_atom2)
            res0_M00 = nquad(M00_integrand, [(0.0, np.inf),(0.0,np.inf)], args_M00)  
            M00[int(l/step),int(l_prime/step)] = res0_M00[0]+0j
            M00[int(l_prime/step),int(l/step)] =M00[int(l/step),int(l_prime/step)]
            
            # Building MQ0 matrix
        for L in range(odd,lmax+1,step):
            args_MQ0_0= (B,k,0,L,l,m,z_atom1,z_atom2)
            res0_MQ0_0_real=  nquad(MQ0_0_integrand, [(0.0, np.inf),(0.0,np.inf)], args_MQ0_0)  
            args_MQ0_0= (B,k,1,L,l,m,z_atom1,z_atom2)
            res0_MQ0_0_imag=  nquad(MQ0_0_integrand, [(0.0, np.inf),(0.0,np.inf)], args_MQ0_0)  
            S_mat[int(l/step),int(L/step)] = res0_MQ0_0_real[0] + 1j*res0_MQ0_0_imag[0]
            
        for col in range ((1+mat_sz),nbasis+(1+mat_sz)):
            #MQ0 where i>0 
            res0_MQ0_i = nquad(MQ0_i_integrand, [(0.0, np.inf),(0.0,np.inf)], sarg_vec[col-(1+mat_sz)]) 
            #print(l,sarg_vec[col-(1+mat_sz)],res0_MQ0_i[0])
            S_mat[col,int(l/2)]=(res0_MQ0_i[0])*N[col-(1+mat_sz)]
    ...
    print("Big_t",big_T)
    #eigen values of T-matrix give the complex tangent of the phase shift
    lmbda_ah = np.linalg.eigvals(big_T)
    print(nbasis)
    #tangents of the phase shift
    lmbda_est = lmbda_ah.imag/lmbda_ah.real
    print("lmbda Estimate: ",lmbda_est)

    #find scattering amplitudes
    theta_in =0     #angle of approach
    theta_out = [theta_in,theta_in+PI/4,theta_in+PI/2]
    phi = 0     #cylindrically symmetric so phi is always 0
    pho = PI    #theta is PI/2 theta_out can't be greater than PI so 
                    #rotate phi by PI to get the other angles
    F = []
    #find f for each theta out
    for tht in theta_out:
        sum_T = 0
        sum_2 = 0
        for lp in range(odd,lmax+1,step):
            
            for l in range(odd,lmax+1,step):
                #useing equations from papers on CKV to find scattering amp
                sum_T = (sum_T+(1j)**(l-lp)*(2/PI)*big_T[int(lp/step),int(l/step)]*
                ...
                sum_2 = (sum_2+(1j)**(l-lp)*(2/PI)*big_T[int(lp/step),int(l/step)]*
